...
def send_pdf_copy_email(agreement, to_email, pdf_url):
    import base64
    import urllib.request
    sg = SendGridAPIClient(settings.SENDGRID_API_KEY)

    # Download from Cloudinary and encode
    with urllib.request.urlopen(pdf_url) as response:
        pdf_data = base64.b64encode(response.read()).decode()
    ...
    mail = Mail(
        ...
        <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 24px;">
          ...
          <h2 style="color:#27500a;">Signed Agreement — Your Copy</h2>
          <p style="color:#424242; font-size:14px; line-height:1.6;">
            Hi <strong>{agreement.contact_name}</strong>,<br><br>
            Please find attached your fully executed copy of
            <strong>{agreement.title}</strong>.
          </p>
          <div style="background:#f5f5f5; border-radius:8px; padding:12px 16px; margin-bottom:24px;">
